...
Paste it into a chat with Claude — it uses the `site_add` tool to store it (encrypted, locally, using your vault key from step 3). From then on you can say things like "select my site" and "create a landing page from this HTML".
...
The `skills/veilwright-workflow.zip` in the folder you downloaded is optional, supplementary detail for edge cases the server's own reference resources (`elementor-widgets`, `fluent-forms`, `privacy-policy`, `workflow-guide` — Claude reads these live from your installed server, on demand, whenever a task needs more detail than the automatic instructions summarize) already expose — most people don't need to install it. If you want it anyway: Claude Desktop → **Settings → Customize → Skills → Upload skill** → select the zip. Unlike the server's own instructions/resources, this copy is a snapshot: re-upload it after an update if you want the latest version.

## Updating to a new version

**The WordPress plugin (Veilwright AI) and this MCP server are two separate things that don't update together.** Updating one doesn't touch the other — a real, confirmed case of confusion: someone updated the WordPress plugin, expected a brand-new tool to show up, and it never did, because this local server was still the old version. If Veilwright AI's changelog mentions a new capability, check whether it also needs a newer `veilwright-mcp` — the plugin's own changelog says so when it does.

**In the folder where you unzipped Veilwright MCP (step 2 above), double-click `update-mac.command` (macOS), `update-windows.bat` (Windows), or run `./update-linux.sh` (Linux).** Unlike the very first setup, this never hits a macOS Gatekeeper block — that file was already approved once, when you ran setup, and updating never introduces a new unsigned executable. It downloads the latest version, replaces the shipped files (your registered sites and their credentials, stored separately in `~/.veilwright/veilwright.sqlite3`, are never touched), reinstalls dependencies, and — unlike just re-running setup — **actually verifies the result works** before telling you it's done, instead of trusting an install step that can fail silently. If it's already the newest version, it says so and changes nothing.

**Then fully quit and reopen Claude Desktop**, same as after setup — an already-running Claude Desktop keeps the old tool list in memory until it's restarted, restarting alone won't help if the update itself didn't happen first (that combination — files never actually updated, then restarting expecting it to help — is exactly what caused the confirmed case above).

If the update script reports a failure, it also reports that your previous install was left untouched — safe to just try again (a `HTTP 429` from GitHub means downloads are being rate-limited from your network right now; wait a minute or two and retry).
...
- **`site_add` works but `site_healthCheck` (or any real tool call) fails to reach the site**: many hosts run a security plugin/firewall that disables the WP REST API by default. Check the WP-admin FAQ in Veilwright AI's `readme.txt` ("I have a security plugin/firewall that disables the REST API") — you'll need to add `/wp-json/veilwright/v1/` as an exception there.
