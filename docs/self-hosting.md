...
- **`npm install` fails on `better-sqlite3` with `fatal error: 'climits' file not found` (macOS)**: your Xcode Command Line Tools are installed but broken or out of date — very common after a macOS update. Fix:
  ```bash
  xcode-select -p   # confirm it's installed at all
  sudo rm -rf /Library/Developer/CommandLineTools
  xcode-select --install
  ```
  Confirm the popup, wait for it to finish installing, then retry `npm install --omit=dev`. If you have full Xcode.app instead of just the Command Line Tools, point to that instead: `sudo xcode-select -s /Applications/Xcode.app/Contents/Developer`.
- **`npm install` fails on `better-sqlite3` for another reason (any OS)**: it needs a C++ compiler to build its native binding. macOS: `xcode-select --install` (see above if that alone doesn't work). Windows: install "Desktop development with C++" via the Visual Studio Build Tools installer. Linux: `sudo apt install build-essential python3` (or your distro's equivalent).
