...
/**
 * Refuses to run inside the source repository. The repo ships this same
 * script, and run there it would copy the published build over the
 * working tree (dist/, skills/, docs/, README.md, package.json) — or, when
 * the repo's version already matches the release, report "already on the
 * latest version" while the real install, the folder Claude Desktop
 * starts, stays old. Both happened to look like a successful update.
 */
function refuseSourceCheckout() {
  const markers = ['src', 'tsconfig.json'].filter((entry) => existsSync(path.join(projectDir, entry)));

  if (markers.length === 0) {
    return;
  }

  log(`✗ This folder is the Veilwright MCP source code (${markers.join(', ')} present), not an installed copy.`);
  log('  Run update-mac.command / update-windows.bat / update-linux.sh from the install folder instead —');
  log('  the one your Claude Desktop config points at (claude_desktop_config.json → mcpServers → veilwright → args).');
  process.exit(1);
}

async function main() {
  refuseSourceCheckout();
