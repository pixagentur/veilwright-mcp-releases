...
export function registerTools(server, deps = buildDefaultDependencies(process.env)) {
    ...
    server.tool('elementorKit_addFont', 'Add a custom Global Font to the active Elementor Kit. `typography` is a flat object of typography_*-prefixed settings (typography_typography, typography_font_family, ' +
        ...
        '`typography_font_family` only actually renders if that font is already available — a system font, a Google Font (referenced by name, no upload needed), or a font uploaded ' +
        'through Elementor Pro\'s separate Custom Fonts screen (WP-admin → Elementor → Custom Elements → Fonts) — there is no API for uploading a font file, that\'s Pro-only and ' +
        'closed-source, so it has to be done manually. If the design just needs a specific typeface and it\'s available as a Google Font, use that name directly instead.', { title: z.string().min(1), typography: z.record(z.string(), z.unknown()), ...SITE_ID_PARAM }, async (args) => {
        try {
            const { siteId, ...body } = args;
            const client = await clients.forSite(tenantId, resolveSiteId(siteId, session));
    ...
    server.tool('menuItem_update', 'Update an existing menu item. Only send the fields that should change. See menuItem_create for the field shapes.', {
        menuId: z.number().int().positive(),
