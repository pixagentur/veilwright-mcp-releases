...
import { WpApiError } from '../../wp-client/WpClient.js';
...
/**
 * `editor_options` and `index` were missing here until 2026-08-13 — zod's
 * default "strip" mode on z.object() silently drops any key not listed,
 * so a caller sending `editor_options` (the exact thing Fluent Forms'
 * editor needs to render a field's settings panel — see
 * veilwright-ai's FluentFormFieldValidator) never reached the WP REST
 * call at all, no error, just silently gone. veilwright-ai now derives
 * `editor_options` itself when omitted, so callers don't have to supply
 * it — but it's declared here too so an explicit override (or a
 * caller-supplied `index`) survives instead of being stripped.
 */
const fluentFormFieldSchema = z.lazy(() => z.object({
    element: z.string().min(1),
    attributes: z.record(z.string(), z.unknown()).optional(),
    settings: z.record(z.string(), z.unknown()).optional(),
    editor_options: z.record(z.string(), z.unknown()).optional().describe('Usually leave unset — veilwright-ai derives the correct value from "element" automatically. Only set to override a specific key (e.g. a custom tile title).'),
...
function errorContent(error) {
    ...
    // The plugin's validation errors carry the actual reasons in `details`
    // (which field, what is wrong, e.g. which taxonomies are registered).
    // Without them the caller only sees "Invalid attributes." and has to guess.
    if (error instanceof WpApiError && error.details.length > 0) {
        message += '\n' + error.details.map(formatErrorDetail).join('\n');
    }
    return { content: [{ type: 'text', text: message }], isError: true };
}
function forceQuery(force) {
    return force === true ? '?force=true' : '';
}
function formatErrorDetail(detail) {
    if (detail !== null && typeof detail === 'object' && 'message' in detail) {
        const { field, message } = detail;
        return typeof field === 'string' && field !== '' ? `- ${field}: ${String(message)}` : `- ${String(message)}`;
    }
...
export function registerTools(server, deps = buildDefaultDependencies(process.env)) {
    ...
    server.tool('site_healthCheck', 'Ping a registered site\'s /system endpoint and record whether it is reachable. The result includes `languages` (null on a single-language site) — ' +
        'check this before creating content and ask the user which language it\'s for if it\'s present, passing that as `lang` to page_create/page_createFromHtml. ' +
        'Also includes `memory` (null on the Free tier or when nothing\'s been written) — if `exists` is true, call siteMemory_get once per conversation before making any change to this site.', { siteId: z.string().min(1) }, async (args) => {
        try {
            ...
            return jsonContent({ ...site, languages: info.languages ?? null, memory: info.memory ?? null, onboarding: info.onboarding ?? null });
        }
        catch (error) {
    ...
    const SLUG_PARENT_PARAMS = {
        slug: z
            .string()
            .optional()
            .describe('URL slug (WordPress\'s post_name). Without this, WordPress derives it from the title, which mangles non-ASCII titles into something ugly (German "Abkanten und Biegen" ' +
            'becomes "abkanten-und-biegen" rather than a shorter, deliberate slug) — set this explicitly whenever the title has umlauts/accents or the URL matters. Not validated ' +
            'for uniqueness or format here; WordPress applies its own sanitization/de-duplication on save.'),
        parent: z
            .number()
            .int()
            .nonnegative()
            .optional()
            .describe("Parent page's ID, for a nested URL (e.g. a page under /leistungen/ becomes /leistungen/laserschneiden/). 0 or omitted means top-level. This only sets the page's own " +
            "hierarchy/URL structure — it does NOT affect a navigation menu's structure at all; a menu item's nesting is controlled separately via menuItem_create/_update's own " +
            '`parentId`.'),
    };
    ...
    const FORCE_DELETE_PARAM = {
        force: z
            .boolean()
            .optional()
            .describe('true deletes permanently, with no way back. Default: moves it to the trash (restorable in WP-admin). The response says which: `status: "trash"` or `"deleted"`.'),
    };
    server.tool('page_delete', 'Move a page to the trash (default) or, with `force: true`, delete it permanently.', { id: z.number().int().positive(), ...FORCE_DELETE_PARAM, ...SITE_ID_PARAM }, async (args) => {
        try {
            const client = await clients.forSite(tenantId, resolveSiteId(args.siteId, session));
            return jsonContent(await client.request('DELETE', `/wp-json/veilwright/v1/pages/${args.id}${forceQuery(args.force)}`));
        }
        catch (error) {
            return errorContent(error);
        }
    ...
    // Shared by post_* and customPostType_*. Patch semantics on update: a key
    // that isn't sent leaves the stored value alone. The response reads every
    // one of these back from the database (`excerpt`, `featuredMedia`, `terms`).
    const POST_RELATION_PARAMS = {
        excerpt: z.string().optional().describe('Excerpt (post_excerpt). "" clears it.'),
        featuredMedia: z
            .number()
            .int()
            .min(0)
            .optional()
            .describe('Featured image (_thumbnail_id): an image attachment ID from the media library (e.g. from media_upload). 0 removes the featured image. ' +
            'Rejected if the ID is not an image attachment.'),
        terms: z
            .record(z.string(), z.array(z.number().int().positive()))
            .optional()
            .describe('Taxonomy terms by taxonomy slug, e.g. {"referenz-kategorie": [25]}. Term IDs only. Each taxonomy listed is REPLACED with exactly these terms ([] clears it); ' +
            'taxonomies not listed are left untouched. Rejected if a taxonomy is not registered for this post type or a term ID does not exist in it — the error lists the registered taxonomies. ' +
            'Check this whenever an Elementor single template is bound to a term (condition like `include/singular/in_<taxonomy>/<term-id>`): without the term the entry renders empty.'),
    };
    ...
    server.tool('post_create', 'Create a regular WordPress post (the built-in Blog Post type — not a page, not an Elementor template). Useful for anything meant to show up as a chronological, ' +
        ...
        'Also takes `excerpt`, `featuredMedia` and `terms` (e.g. tags as {"post_tag": [ids]}); categories go either in `categories` or in `terms.category`, not both. The response is read back from the database.', {
        title: z.string().min(1),
        content: z.string().optional(),
        status: z.string().optional(),
        lang: z
        ...
        slug: z
            .string()
            .optional()
            .describe('URL slug (WordPress\'s post_name). Without this, WordPress derives it from the title, which mangles non-ASCII titles into something ugly — set this explicitly ' +
            'whenever the title has umlauts/accents or the URL matters.'),
        ...DATE_PARAM,
        ...POST_CATEGORIES_PARAM,
        ...POST_RELATION_PARAMS,
        ...SITE_ID_PARAM,
    }, async (args) => {
        try {
            const { siteId, ...attributes } = args;
    ...
    server.tool('post_update', 'Update a WordPress post on a site. PATCH semantics: only the fields you send change. Also takes `excerpt`, `featuredMedia` and `terms` (see post_create); the response is read back from the database.', {
        id: z.number().int().positive(),
        title: z.string().optional(),
        content: z.string().optional(),
        status: z.string().optional(),
        ...
        translationOfId: z
            ...
            .optional()
            .describe('An existing post ID this one is a translation of — links them as a translation group. Requires `lang` to also be set.'),
        slug: z.string().optional().describe('URL slug (WordPress\'s post_name).'),
        ...DATE_PARAM,
        ...POST_CATEGORIES_PARAM,
        ...POST_RELATION_PARAMS,
        ...SITE_ID_PARAM,
    ...
    server.tool('post_delete', 'Move a WordPress post to the trash (default) or, with `force: true`, delete it permanently.', { id: z.number().int().positive(), ...FORCE_DELETE_PARAM, ...SITE_ID_PARAM }, async (args) => {
        try {
            const client = await clients.forSite(tenantId, resolveSiteId(args.siteId, session));
            return jsonContent(await client.request('DELETE', `/wp-json/veilwright/v1/posts/${args.id}${forceQuery(args.force)}`));
        }
        catch (error) {
            return errorContent(error);
        }
    ...
    server.tool('customPostType_create', 'Create an entry of a custom post type the site owner already registered themselves (via ACF, code, another plugin — not a page, not the built-in Post type, not an Elementor template). ' +
        "Pass its exact registered slug as `postType` (ask the user or check WP-admin if unsure). Sets the standard title/content/status/language/slug fields, plus `parent` if the post type " +
        'was registered hierarchical (has no effect otherwise), plus `excerpt`, `featuredMedia` (featured image) and `terms` (taxonomy terms) — so an entry can be created complete in one call. ' +
        'The response is read back from the database after saving, including `excerpt`, `featuredMedia` and every taxonomy registered for the type under `terms` (empty lists included): check it. ' +
        'Any field specific to the custom post type itself (ACF fields, etc.) must be set afterward with `acf_update`/`acf_updateAll` on the returned `id`, which work on any post regardless of its post type.', {
    ...
    server.tool('customPostType_update', 'Update an entry of a custom post type. This is a PATCH: only the fields you send change, everything else (content, excerpt, featured image, terms of taxonomies you do not list) stays as stored. ' +
        'Covers title/content/status/language/slug/parent/date plus `excerpt`, `featuredMedia` and `terms`; the response is read back from the database. ' +
        'Use acf_update/acf_updateAll for fields specific to the post type itself.', {
        postType: z.string().min(1).describe('The custom post type\'s exact registered slug.'),
        id: z.number().int().positive(),
        title: z.string().optional(),
        content: z.string().optional(),
    ...
    server.tool('customPostType_delete', 'Move an entry of a custom post type to the trash (default) or, with `force: true`, delete it permanently. ' +
        'Needs veilwright-ai 0.45.0 or later on the site: older versions delete custom post type entries permanently even without `force` — check the response\'s `status`.', {
        postType: z.string().min(1).describe('The custom post type\'s exact registered slug.'),
        id: z.number().int().positive(),
        ...FORCE_DELETE_PARAM,
        ...SITE_ID_PARAM,
        ...
        try {
            const client = await clients.forSite(tenantId, resolveSiteId(args.siteId, session));
            return jsonContent(await client.request('DELETE', `/wp-json/veilwright/v1/custom-post-types/${encodeURIComponent(args.postType)}/${args.id}${forceQuery(args.force)}`));
        }
        catch (error) {
            return errorContent(error);
        }
    ...
    server.tool('siteMemory_get', 'Get a site\'s standing Memory note (Pro tier only — 403 VW_LICENSE_TIER_REQUIRED on Free tier) — a persistent free-text instruction the site owner wrote for the AI to check before making any change (e.g. "never touch the pricing page without asking first", "keep the tone formal"). ' +
        'Cheapest way to know whether one exists without calling this: site_healthCheck\'s `memory.exists` field. Treat `content` as binding context, not optional background reading.', { ...SITE_ID_PARAM }, async (args) => {
        try {
            const client = await clients.forSite(tenantId, resolveSiteId(args.siteId, session));
            return jsonContent(await client.request('GET', '/wp-json/veilwright/v1/site-memory'));
        }
        catch (error) {
            return errorContent(error);
        }
    });
    server.tool('siteMemory_update', 'Replace a site\'s standing Memory note (Pro tier only — 403 VW_LICENSE_TIER_REQUIRED on Free tier). Whole-content replace, not an append — read the current content first with siteMemory_get if adding to an existing note rather than overwriting it.', {
        content: z.string().describe('The full new Memory note content. Pass an empty string to clear it.'),
        ...SITE_ID_PARAM,
    }, async (args) => {
        try {
            const client = await clients.forSite(tenantId, resolveSiteId(args.siteId, session));
            return jsonContent(await client.request('PUT', '/wp-json/veilwright/v1/site-memory', { content: args.content }));
        }
        catch (error) {
            return errorContent(error);
        }
    });
    server.tool('pageBlueprint_list', 'List available page blueprints ("Webseite vervollständigen" — currently search-results, error-404) and whether each already exists on this site. ' +
        'For any blueprint whose `imagePrompt` is non-null: that text is a static, per-site-unaware fallback (no LLM runs in that code path, it only knows the site\'s tagline and Kit colors) — don\'t just pass it through. ' +
        'Build a real prompt yourself instead: read the site\'s actual content (WebFetch/browser tools, page_list/page_get) and its Kit brand colors (elementorKit_listColors), then write a Midjourney-formula prompt — ' +
        'Subject → Setting → Style/Medium → Parameters, short descriptive phrases not sentences. Exactly one concrete subject, never "X or Y" (Midjourney renders the literal text, it never chooses between alternatives). ' +
        'Put exclusions in a trailing `--no` param as single words (e.g. `--no text, watermark, logo`), not inline "no text" phrasing — `--no` is parsed word-by-word, so multi-word terms there get split and over-applied. ' +
        'Earlier words carry more weight, so put the site-specific content first. Upload the resulting image with media_upload, then pass its returned id as `imageAttachmentId` to pageBlueprint_create.', { ...SITE_ID_PARAM }, async (args) => {
        try {
            const client = await clients.forSite(tenantId, resolveSiteId(args.siteId, session));
            return jsonContent(await client.request('GET', '/wp-json/veilwright/v1/page-blueprints'));
        }
        catch (error) {
            return errorContent(error);
        }
    });
    server.tool('pageBlueprint_create', 'Create a page blueprint\'s main-language page (e.g. the 404 page). Call pageBlueprint_list first — this fails if the blueprint already exists, or (for Elementor-Pro-only blueprints) Elementor Pro isn\'t active on this site. ' +
        '`imageAttachmentId` is optional and should be the id media_upload returned for an image generated from a properly-built prompt (see pageBlueprint_list\'s description for what "properly-built" means) — the layout is still solid without one.', {
        slug: z.string().min(1).describe('A slug from pageBlueprint_list, e.g. "error-404".'),
        imageAttachmentId: z.number().int().positive().optional(),
        ...SITE_ID_PARAM,
    }, async (args) => {
        try {
            const client = await clients.forSite(tenantId, resolveSiteId(args.siteId, session));
            ...
            if (args.imageAttachmentId !== undefined)
                body.imageAttachmentId = args.imageAttachmentId;
            return jsonContent(await client.request('POST', `/wp-json/veilwright/v1/page-blueprints/${encodeURIComponent(args.slug)}`, body));
        }
        catch (error) {
            return errorContent(error);
        }
    });
    server.tool('pageBlueprint_createTranslations', 'Create translations of an already-created page blueprint (same layout, translated text) for one or more languages. `langs` must be a subset of that blueprint\'s `missingTranslationLanguages` from pageBlueprint_list. ' +
        'One optional `imageAttachmentId` applies to every selected language (same illustration reused across translations, the common case).', {
        slug: z.string().min(1).describe('A slug from pageBlueprint_list, e.g. "error-404".'),
        langs: z.array(z.string().min(1)).min(1).describe('Language codes, each one from that blueprint\'s missingTranslationLanguages.'),
        imageAttachmentId: z.number().int().positive().optional(),
        ...SITE_ID_PARAM,
    }, async (args) => {
        try {
            const client = await clients.forSite(tenantId, resolveSiteId(args.siteId, session));
            const body = { langs: args.langs };
            if (args.imageAttachmentId !== undefined)
                body.imageAttachmentId = args.imageAttachmentId;
            return jsonContent(await client.request('POST', `/wp-json/veilwright/v1/page-blueprints/${encodeURIComponent(args.slug)}/translations`, body));
        }
        catch (error) {
            return errorContent(error);
        }
    ...
    server.tool('page_getElementorData', "Read a page's actual, currently-persisted Elementor element tree (`_elementor_data`) — independent of frontend rendering (which depends on caching/theme/CSS regeneration) " +
        ...
        "`page_get`'s `content` field is never this — that's the classic WordPress post_content field, which Elementor leaves empty on purpose. A real page's full data can be " +
        'very large — pass `structureOnly: true` when only the shape is needed (which ids/types nest where, and each element\'s `_title` if one was set), not any setting\'s value.', {
        id: z.number().int().positive().describe('The page to read.'),
        structureOnly: z.boolean().optional().describe('If true, drops every settings value, keeping only id/elType/widgetType/_title/nesting.'),
        ...SITE_ID_PARAM,
    }, async (args) => {
        try {
            const client = await clients.forSite(tenantId, resolveSiteId(args.siteId, session));
            const query = args.structureOnly ? '?structureOnly=true' : '';
            return jsonContent(await client.request('GET', `/wp-json/veilwright/v1/pages/${args.id}/elementor/elements${query}`));
        }
        catch (error) {
            return errorContent(error);
        }
    ...
    const positionSchema = z
        .union([z.number().int().min(0), z.enum(['start', 'end'])])
        .optional()
        .describe('0-based index among the target\'s current children, or "start"/"end". Default "end". At most one of position/beforeId/afterId.');
    server.tool('page_addElementorWidget', 'Append a specific Elementor widget (or a container with nested widgets) to an existing page — use this for widget types page_createFromHtml has no native mapping for ' +
        ...
        'scratch — use page_createFromHtml or page_create for that, then add specific widgets with this afterwards. By default the element is appended to the page\'s top level, ' +
        'unchanged from before parentId/position/beforeId/afterId existed — pass one of them to insert into a specific existing container, or between two existing siblings, ' +
        'instead. Prefer beforeId/afterId (a neighbor\'s real id, e.g. from page_getElementorData) over position when you have one — an index has to be counted and is silently ' +
        "wrong the moment anything nearby changes; beforeId/afterId find their own parent automatically, parentId isn't needed alongside them just to locate the target. An " +
        "unresolvable parentId/beforeId/afterId or an out-of-range position is always a real error, never a silent fall-back to the end.", {
        ...
        parentId: z.string().min(1).optional().describe('Id of an existing container to insert into. Omitted = the page\'s top level.'),
        position: positionSchema,
        beforeId: z.string().min(1).optional().describe("Insert immediately before this existing sibling's id. Its parent is found automatically."),
        afterId: z.string().min(1).optional().describe("Insert immediately after this existing sibling's id. Its parent is found automatically."),
        ...SITE_ID_PARAM,
    }, async (args) => {
        try {
            const client = await clients.forSite(tenantId, resolveSiteId(args.siteId, session));
            const body = { ...args.element };
            if (args.parentId !== undefined)
                body.parentId = args.parentId;
            if (args.position !== undefined)
                body.position = args.position;
            if (args.beforeId !== undefined)
                body.beforeId = args.beforeId;
            if (args.afterId !== undefined)
                body.afterId = args.afterId;
            return jsonContent(await client.request('POST', `/wp-json/veilwright/v1/pages/${args.id}/elementor/elements`, body));
        }
        catch (error) {
            return errorContent(error);
        }
    ...
    server.tool('page_updateElementorWidget', 'Fix an existing Elementor widget/container on a page in place — e.g. a typo, a wrong link, a color — without resending the whole element or rebuilding the page. ' +
        '`elementId` is that element\'s own `id` (returned by page_addElementorWidget when it was created, or from page_createFromHtml\'s conversion — use page_getElementorData ' +
        'if the id wasn\'t kept track of). `settings` is shallow-merged into the element\'s existing settings — ' +
        'only the keys you pass change, everything else stays. Cannot change `elType`/`widgetType`/child elements this way — use page_replaceElementorElement instead if the ' +
        'widget type itself needs to change (e.g. a text-editor holding a hand-written list → a real icon-list widget), not just its settings.', {
    ...
    server.tool('page_replaceElementorElement', 'Replace an existing Elementor element (and everything nested inside it) with a new one, at the exact same position — the one thing page_updateElementorWidget ' +
        'deliberately can\'t do (it can only shallow-merge `settings`, never change `elType`/`widgetType`/children). Use this when an element\'s own type/structure needs to ' +
        'change, e.g. a text-editor holding a hand-written `<ul>` → a real icon-list widget — one call, no positioning needed afterward, unlike deleting the old element and ' +
        'adding a new one (which would land at the end of the page, not back where the old one was). If the replaced element is genuinely at the page\'s top level, a bare ' +
        '`widget` in `element` is auto-wrapped in a container the same way page_addElementorWidget does — nested elsewhere, no such wrapping happens (a container holding a ' +
        'widget directly is completely normal there). Always snapshots the page first — same mechanism as page_deleteElementorElement.', {
        id: z.number().int().positive().describe('The page the element lives on.'),
        elementId: z.string().min(1).describe("The element's own id to replace (not the page id)."),
        element: elementorElementSchema.describe('The replacement element — same shape as page_addElementorWidget\'s element.'),
        ...SITE_ID_PARAM,
    }, async (args) => {
        try {
            const client = await clients.forSite(tenantId, resolveSiteId(args.siteId, session));
            return jsonContent(await client.request('POST', `/wp-json/veilwright/v1/pages/${args.id}/elementor/elements/${args.elementId}/replace`, { element: args.element }));
        }
        catch (error) {
            return errorContent(error);
        }
    });
    server.tool('page_moveElementorElement', 'Move an existing Elementor element (and everything nested inside it) to a different position — a different container, a different spot among its current siblings, ' +
        'or both — without resending its content. Same parentId/position/beforeId/afterId locators page_addElementorWidget uses to choose where an inserted element lands ' +
        '(prefer beforeId/afterId over position for the same reason: an index has to be counted and is silently wrong the moment anything nearby changes). Refuses to move an ' +
        'element into itself or into one of its own descendants. Always snapshots the page first — same mechanism as page_deleteElementorElement.', {
        id: z.number().int().positive().describe('The page the element lives on.'),
        elementId: z.string().min(1).describe("The element's own id to move (not the page id)."),
        parentId: z.string().min(1).optional().describe('Id of the container to move it into. Omitted = the page\'s top level.'),
        position: positionSchema,
        beforeId: z.string().min(1).optional().describe("Move it to immediately before this existing sibling's id. Its parent is found automatically."),
        afterId: z.string().min(1).optional().describe("Move it to immediately after this existing sibling's id. Its parent is found automatically."),
        ...SITE_ID_PARAM,
    }, async (args) => {
        try {
            const client = await clients.forSite(tenantId, resolveSiteId(args.siteId, session));
            ...
            if (args.afterId !== undefined)
                body.afterId = args.afterId;
            return jsonContent(await client.request('POST', `/wp-json/veilwright/v1/pages/${args.id}/elementor/elements/${args.elementId}/move`, body));
        }
        catch (error) {
            return errorContent(error);
        }
    });
    server.tool('page_replaceElementorData', 'Replace a page\'s ENTIRE Elementor element tree in one atomic write — for swapping a page\'s content wholesale (a redesign, rebuilding a Datenschutzerklärung) instead of ' +
        'removing/adding elements one at a time. Standard REST semantics: page_addElementorWidget appends one element, this replaces the whole collection. Each top-level entry ' +
        'in `elements` goes through the same validation/wrapping page_addElementorWidget uses — a bare top-level "widget" gets auto-wrapped in a container. An empty array is a ' +
        'legitimate call: it clears the page. This is a real, immediate write with no built-in undo — call page_backup first, always, before using this.', {
        id: z.number().int().positive().describe('The page to replace the content of.'),
        elements: z.array(elementorElementSchema).describe('Top-level Elementor elements — same shape as page_addElementorWidget\'s element. Empty array clears the page.'),
        ...SITE_ID_PARAM,
    }, async (args) => {
        try {
            const client = await clients.forSite(tenantId, resolveSiteId(args.siteId, session));
            return jsonContent(await client.request('PUT', `/wp-json/veilwright/v1/pages/${args.id}/elementor/elements`, { elements: args.elements }));
        }
        catch (error) {
            return errorContent(error);
        }
    });
    server.tool('page_deleteElementorElement', "Remove one or more existing Elementor widgets/containers from a page entirely — the counterpart page_updateElementorWidget can't provide, since that one can only " +
        "change an existing element's settings, never remove it. Removes everything nested inside each too if it's a container with children. `elementId` is the element's " +
        "own id (same one page_updateElementorWidget/page_addElementorWidget use — use page_getElementorData first if you need to find it) — pass an array to remove several " +
        'in one call/one write, which is the common case for real cleanup (a heading plus its body text, a dozen widgets of an old section) and far cheaper than one call per ' +
        'element. A pre-delete backup snapshot is taken automatically every time (not optional) — the response\'s `backupId` is the way back, this call itself has no undo. ' +
        'Set `dryRun: true` to see exactly what would be removed (same response shape, nothing written, no backup taken) before committing — worth doing whenever the user ' +
        'might be surprised by what matches. Succeeds even if some (not all) requested ids don\'t exist — those come back in `notFound` rather than failing the whole call, so ' +
        'a retried/repeated call is safe. Fails if a requested id occurs more than once anywhere in the document (ambiguous, likely document corruption) or if none of the ' +
        'requested ids matched anything.', {
        id: z.number().int().positive().describe('The page the element(s) live on.'),
        elementId: z
            .union([z.string().min(1), z.array(z.string().min(1)).min(1)])
            .describe("One element id, or an array of several to remove together in one write."),
        dryRun: z.boolean().optional().describe('If true, report what would be removed without writing anything or taking a backup.'),
        ...SITE_ID_PARAM,
    }, async (args) => {
        try {
            const client = await clients.forSite(tenantId, resolveSiteId(args.siteId, session));
            const body = { elementId: args.elementId };
            if (args.dryRun !== undefined)
                body.dryRun = args.dryRun;
            return jsonContent(await client.request('DELETE', `/wp-json/veilwright/v1/pages/${args.id}/elementor/elements`, body));
        }
        catch (error) {
            return errorContent(error);
        }
    ...
    server.tool('elementorTemplate_list', 'List Theme Builder templates without their content — each item has id/title/type/lang. Use this to check which languages already have a template of a given type ' +
        ...
        type: z
            .enum(['footer', 'header', 'error-404', 'search-results', 'single-post', 'product', 'popup', 'archive', 'loop-item'])
            .optional()
    ...
    server.tool('elementorTemplate_create', 'Create a conditional Elementor Theme Builder template (footer, header, 404, search-results, single-post, product, popup, archive, or loop-item) — for a request naming ' +
        "one of these, use this instead of page_create/page_addElementorWidget, which would only edit a single page's content, not a real site-wide/conditional template. Build " +
        "`elements` the same way as page_addElementorWidget's `element` (see ELEMENTOR_WIDGETS.md — for single-post/product templates, use context-only widgets like " +
        'theme-post-content/woocommerce-*; for the content-type-backed-section pair of `archive`+`loop-item`, see the `content-type-sections` resource). `conditions` only ' +
        'defaults to entire-site ("include/general") for footer/header; `loop-item` never takes one at all (it\'s referenced directly by a `loop-grid` widget\'s `template_id`, ' +
        'never matched by the condition system — omit `conditions` for it); every other type, including `archive`, REQUIRES an explicit condition string. Elementor Pro\'s ' +
        'conditions are a TREE in most confirmed cases, but not reliably — two real, independently-verified sites disagreed on whether an archive condition needs the `archive/` ' +
        'segment (`["include/archive/<slug>_archive"]` on one site, flat `["include/<slug>_archive"]` on another) — don\'t assume either shape holds for a new site, check the ' +
        'sibling doc elementor-templates.md\'s table for what else is confirmed vs. an informed guess, and when it matters, read the site\'s own live condition config rather ' +
        "than guessing. For a popup, `pageSettings` covers width/close-button/background only — its trigger (on page load, on scroll, ...) has no ground truth yet and needs " +
        "setting manually in the editor's Popup Settings → Triggers tab; always tell the user this after creating one, an untriggered popup never shows. On a multilingual site " +
        "(Polylang/WPML), pass `lang` explicitly — templates otherwise land in whatever language Polylang defaults new elementor_library posts to (its own site default, not " +
        "necessarily the language the template is actually for), and there's no other way to set it at creation time.", {
        title: z.string().min(1),
        type: z.enum(['footer', 'header', 'error-404', 'search-results', 'single-post', 'product', 'popup', 'archive', 'loop-item']),
        ...
        conditions: z
            .array(z.string())
            .optional()
            .describe('Elementor Pro display-condition strings. Defaults to ["include/general"] for footer/header only; not applicable at all for loop-item; required for every other type.'),
        pageSettings: z
    ...
    server.tool('elementorTemplate_get', 'Read back a Theme Builder template created via elementorTemplate_create (id, title, type, conditions, pageSettings, elements, lang). A real template\'s full `elements` ' +
        'can be very large (one real read was 151,428 characters and overflowed the response limit) — pass `structureOnly: true` when only the shape is needed (which ids/types ' +
        'nest where, and each element\'s `_title` if one was set), not any setting\'s actual value.', {
        id: z.number().int().positive(),
        structureOnly: z.boolean().optional().describe('If true, `elements` drops every settings value, keeping only id/elType/widgetType/_title/nesting.'),
        ...SITE_ID_PARAM,
    }, async (args) => {
        try {
            const client = await clients.forSite(tenantId, resolveSiteId(args.siteId, session));
            const query = args.structureOnly ? '?structureOnly=true' : '';
            return jsonContent(await client.request('GET', `/wp-json/veilwright/v1/elementor/templates/${args.id}${query}`));
        }
        catch (error) {
            return errorContent(error);
        }
    ...
    server.tool('designSystem_get', 'Get a site\'s Design System — colors, typography, spacing, border-radius, shadows, button-style guidance, icon style, imagery/voice-tone guidance, and logo (Pro tier only — 403 VW_LICENSE_TIER_REQUIRED on Free tier). ' +
        'This is separate from the Elementor Kit (elementorKit_*) — sync between the two is explicit and manual via designSystem_syncFromKit/designSystem_syncToKit, never automatic.', { ...SITE_ID_PARAM }, async (args) => {
        try {
            const client = await clients.forSite(tenantId, resolveSiteId(args.siteId, session));
            return jsonContent(await client.request('GET', '/wp-json/veilwright/v1/design-system'));
        }
        catch (error) {
            return errorContent(error);
        }
    });
    server.tool('designSystem_update', 'Update a site\'s Design System (Pro tier only — 403 VW_LICENSE_TIER_REQUIRED on Free tier). Whichever whole top-level group is passed REPLACES that entire group — e.g. passing `colors` replaces all of colors, not just the fields given within it (not a per-field deep merge). ' +
        'Only pass the groups that should change; call designSystem_get first if adding to an existing group (e.g. appending one more custom color) rather than overwriting it.', {
        colors: z
            .object({
            ...
            .optional()
            .describe('Hex color strings, e.g. "#AA5843".'),
        typography: z
            .object({
            headingFont: z.string().optional(),
            bodyFont: z.string().optional(),
            scale: z
                .array(z.object({ name: z.string(), sizePx: z.number(), lineHeight: z.number().optional(), weight: z.number().optional() }))
                .optional(),
        })
            .optional(),
        spacing: z.array(z.object({ name: z.string(), valuePx: z.number() })).optional(),
        radii: z.array(z.object({ name: z.string(), valuePx: z.number() })).optional(),
        shadows: z.array(z.object({ name: z.string(), value: z.string() })).optional().describe('value is a raw CSS box-shadow string.'),
        buttons: z.object({ variants: z.array(z.object({ name: z.string(), description: z.string() })) }).optional(),
        icons: z.object({ style: z.enum(['outline', 'filled', 'line']).optional(), strokeWidthPx: z.number().optional() }).optional(),
        imagery: z.string().optional().describe('Free-text imagery/photography style guidance.'),
        voiceTone: z.string().optional().describe('Free-text brand voice/tone guidance for content generation.'),
        ...SITE_ID_PARAM,
    }, async (args) => {
        try {
            const client = await clients.forSite(tenantId, resolveSiteId(args.siteId, session));
            const { siteId: _siteId, ...patch } = args;
            return jsonContent(await client.request('PUT', '/wp-json/veilwright/v1/design-system', patch));
        }
        catch (error) {
            return errorContent(error);
        }
    });
    server.tool('designSystem_uploadLogo', 'Upload a local image file as the site\'s Design System logo (Pro tier only — 403 VW_LICENSE_TIER_REQUIRED on Free tier). Replaces any existing logo.', { filePath: z.string().min(1), ...SITE_ID_PARAM }, async (args) => {
        try {
            const client = await clients.forSite(tenantId, resolveSiteId(args.siteId, session));
            const data = await readFile(args.filePath);
            const filename = basename(args.filePath);
            const result = await client.uploadFile('/wp-json/veilwright/v1/design-system/logo', {}, {
                fieldName: 'logo',
                filename,
    ...
    server.tool('designSystem_removeLogo', 'Remove a site\'s Design System logo (Pro tier only — 403 VW_LICENSE_TIER_REQUIRED on Free tier).', { ...SITE_ID_PARAM }, async (args) => {
        try {
            const client = await clients.forSite(tenantId, resolveSiteId(args.siteId, session));
            return jsonContent(await client.request('DELETE', '/wp-json/veilwright/v1/design-system/logo'));
        }
        catch (error) {
            return errorContent(error);
        }
    });
    server.tool('designSystem_syncFromKit', 'Pull the active Elementor Kit\'s current colors/fonts INTO the Design System — one-shot, explicit (Pro tier only — 403 VW_LICENSE_TIER_REQUIRED on Free tier; also fails if the site has no active Elementor Kit). ' +
        'Lossy by nature: Elementor only has primary/secondary/text/accent system color slots, no semantic success/warning/error/info equivalents — those aren\'t touched. Never called automatically; only run this when the user explicitly asks to import from the Kit.', { ...SITE_ID_PARAM }, async (args) => {
        try {
            const client = await clients.forSite(tenantId, resolveSiteId(args.siteId, session));
            return jsonContent(await client.request('POST', '/wp-json/veilwright/v1/design-system/sync-from-kit'));
        }
        catch (error) {
            return errorContent(error);
        }
    });
    server.tool('designSystem_syncToKit', 'Push the Design System\'s colors/fonts INTO the active Elementor Kit — one-shot, explicit, OVERWRITES the Kit\'s current primary/secondary/accent colors and system fonts (Pro tier only — 403 VW_LICENSE_TIER_REQUIRED on Free tier; also fails if the site has no active Elementor Kit). ' +
        'Never called automatically; only run this when the user explicitly asks to write into the Kit — confirm first if it wasn\'t clearly requested, since it overwrites existing Kit values.', { ...SITE_ID_PARAM }, async (args) => {
        try {
            const client = await clients.forSite(tenantId, resolveSiteId(args.siteId, session));
            return jsonContent(await client.request('POST', '/wp-json/veilwright/v1/design-system/sync-to-kit'));
        }
        catch (error) {
            return errorContent(error);
        }
    ...
    server.tool('form_createFluentForm', 'Build a brand-new Fluent Forms form (fields, submit button, settings, notifications) — for embedding a form that already exists instead, use the fluent-form-widget via ' +
        ...
        submitButton: z
            .union([z.string().min(1), fluentFormFieldSchema])
            .optional()
            .describe('Either the button\'s visible text as a plain string (recommended — e.g. "Jetzt anfragen"), or a full field object to also override styling. Defaults to a plain "Submit Form" button if omitted.'),
    ...
    server.tool('codeSnippet_list', 'List all Code Snippets (codesnippets.pro) snippets on a site — custom PHP/CSS/JS/HTML snippets, not Elementor widgets or Theme Builder templates. Requires the Code ' +
        'Snippets plugin active on the site. Includes trashed snippets too (`active: false` — Code Snippets soft-deletes, see codeSnippet_delete). Every snippet\'s full `code` ' +
        'is sent by default, which can add up fast (31 real snippets on one site: 70,407 characters) — pass `includeCode: false` when only name/scope/active is needed to decide ' +
        'what to do next; fetch the full snippet with codeSnippet_get afterward for the one that actually matters.', {
        includeCode: z.boolean().optional().describe('Default true. Set false to omit each snippet\'s `code` field and shrink the response.'),
        ...SITE_ID_PARAM,
    }, async (args) => {
        try {
            const client = await clients.forSite(tenantId, resolveSiteId(args.siteId, session));
            const query = args.includeCode === false ? '?includeCode=false' : '';
            return jsonContent(await client.request('GET', `/wp-json/veilwright/v1/code-snippets${query}`));
        }
        catch (error) {
            return errorContent(error);
        }
    ...
    server.tool('codeSnippet_delete', 'Delete a Code Snippets snippet. Trashes by default — a real, confirmed soft-delete (Code Snippets\' own `trash_snippet()`): the snippet keeps appearing in codeSnippet_list/' +
        'codeSnippet_get afterward with `active: false`, not actually gone — that\'s Code Snippets\' own intended behavior (it has a separate Trash view), not a bug. Pass ' +
        '`permanent: true` for a real, irreversible delete.', {
        id: z.number().int().positive(),
        permanent: z.boolean().optional().describe('If true, delete for real instead of trashing.'),
        ...SITE_ID_PARAM,
    }, async (args) => {
        try {
            const client = await clients.forSite(tenantId, resolveSiteId(args.siteId, session));
            ...
            if (args.permanent !== undefined)
                body.permanent = args.permanent;
            return jsonContent(await client.request('DELETE', `/wp-json/veilwright/v1/code-snippets/${args.id}`, body));
        }
        catch (error) {
            return errorContent(error);
        }
    ...
    server.tool('menuItem_create', '`type` is REQUIRED for every item, not just an optional hint — omitting it (even with `objectType`/`objectId` correctly set) creates a dead "custom" link with an empty URL ' +
        "that WordPress does not fill in for you; this is a real, confirmed bug in how WordPress's own wp_insert_post/nav-menu-item plumbing behaves, not something this tool can " +
        'paper over. Three shapes, pick the matching `type` explicitly every time: ' +
        '(1) custom link — `type: "custom"`, set `url`, leave `objectType`/`objectId` unset. ' +
        '(2) existing page/post/category/tag — `type: "post_type"` (page/post and most CPTs) or `type: "taxonomy"` (category/tag/custom taxonomy terms), plus `objectType` (e.g. ' +
        '"page", "post", "category") and `objectId` — `url` is then derived by WordPress itself. ' +
        '(3) a custom post type\'s archive page — `type: "post_type_archive"`, `objectType` set to the CPT slug, and `objectId`/`url` both left unset (WordPress derives the archive ' +
        'URL from the post type registration, not from any specific post). Do not link a same-named regular page instead — its slug collides with the archive\'s own slug, and the ' +
        'menu item breaks (shows "(ungültig)"/invalid) the moment that page is ever deleted. ' +
        '`parentId` nests this item as a submenu entry under another item\'s ID; `position` controls display order (WordPress\'s own `menu_order`, ties broken by creation order). ' +
        ...
        "The create/update response itself is not reliable confirmation (see menuItem_update) — verify with menuItem_list afterwards.", {
        ...
        type: z
            .string()
            .describe('REQUIRED, not optional in practice — see tool description. "custom" for a raw URL, "post_type" for page/post/CPT, "taxonomy" for category/tag/custom taxonomy, "post_type_archive" for a CPT\'s archive page.'),
        objectType: z.string().optional().describe('e.g. "page", "post", "category", or a CPT slug for post_type_archive — set alongside `type`.'),
        objectId: z.number().int().optional().describe('Required for post_type/taxonomy; omit for custom and post_type_archive.'),
    ...
    server.tool('menuItem_update', 'Update an existing menu item. THIS IS A REPLACE, NOT A PATCH, despite every field being individually optional in this schema — WordPress core\'s own ' +
        'wp_update_nav_menu_item() resets any menu-item-* argument you don\'t pass to an empty default rather than preserving the item\'s current value for it. A call with only ' +
        '`title` set has, in practice, wiped `parentId` (a submenu item jumps to the top level), `titleAttribute`, and the object link on real sites. Always send the COMPLETE ' +
        'current field set — `title`, `type`, `objectType`, `objectId`, `parentId`, `position`, `titleAttribute` — even for fields you\'re not changing; fetch the current values ' +
        'via menuItem_list first if you don\'t already have them. See menuItem_create for what each field means. If this item has no `titleAttribute` yet, set one now — ' +
        'SEO-optimized, in the menu\'s actual language — rather than leaving it missing. The response from this call is not reliable confirmation — verify with menuItem_list.', {
        menuId: z.number().int().positive(),
    ...
    server.tool('site_inventory', 'Fetch a site\'s own real, rendered frontend URLs and analyze them for external hosts (split into embedded resource vs. mere link — the single most consequential ' +
        'distinction a copied privacy-policy template gets wrong), frontend plugin fingerprints, iframe count, form fields (incl. file-upload detection), and whether a native ' +
        'WordPress comment form is present. Encapsulates PRIVACY_POLICY.md\'s manual "Schritt 1 — Inventar erheben" into one call — see that file in this skill for the full ' +
        'workflow this feeds into. Pick `urls` yourself (via page_list or what you already know about the site) — at minimum the homepage, contact page, one content page, the ' +
        'address/location page, and every page with a form; the tool deliberately does not guess slugs like "kontakt"/"anfahrt" itself. Every URL must resolve to the site\'s ' +
        'own host or it\'s skipped (with a reason in the response) — this cannot fetch arbitrary external URLs. Analyzes real rendered HTML, not post_content, which is often ' +
        'empty on Elementor-built pages.', {
        urls: z.array(z.string().url()).min(1).max(20).describe('The site\'s own real frontend page URLs to fetch and analyze, 1-20 entries.'),
        ...SITE_ID_PARAM,
    }, async (args) => {
        try {
            const client = await clients.forSite(tenantId, resolveSiteId(args.siteId, session));
            return jsonContent(await client.request('POST', '/wp-json/veilwright/v1/site-inventory', { urls: args.urls }));
        }
        catch (error) {
            return errorContent(error);
        }
