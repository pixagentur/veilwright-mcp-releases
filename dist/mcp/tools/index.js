...
export function registerTools(server, deps = buildDefaultDependencies(process.env)) {
    ...
    server.tool('site_healthCheck', 'Ping a registered site\'s /system endpoint and record whether it is reachable.', { siteId: z.string().min(1) }, async (args) => {
        try {
    ...
    server.tool('page_create', 'Create a page on a site.', { title: z.string().min(1), content: z.string().optional(), status: z.string().optional(), ...SITE_ID_PARAM }, async (args) => {
        try {
