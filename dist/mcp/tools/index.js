...
export function registerTools(server, deps = buildDefaultDependencies(process.env)) {
    ...
    server.tool('elementorTemplate_create', 'Create a conditional Elementor Theme Builder template (footer, header, 404, search-results, single-post, product, or popup) — for a request naming one of these, use this ' +
        ...
        '`conditions` only defaults to entire-site ("include/general") for footer/header; every other type REQUIRES an explicit condition string. Elementor Pro\'s conditions are a ' +
        'TREE, not a flat namespace, so the right string is not always guessable from the type name — see the sibling doc elementor-templates.md for the full table, confirmed ' +
        'live against real editor-built templates: 404 is `["include/singular/not_found404"]`, search-results is `["include/archive/search"]`, a custom-post-type SINGLE template ' +
        'is flat (`["include/<slug>"]`, e.g. `["include/stellenanzeige"]` — NOT `include/singular/post-type/<slug>`), and a custom-post-type ARCHIVE template needs the archive ' +
        'segment (`["include/archive/<slug>_archive"]`) — ask the user if a condition isn\'t covered there rather than guessing. `archive` and `loop-item` are not creatable ' +
        "through this tool at all (see its `type` enum) — tell the user upfront to build those in the Elementor editor instead of trying either value (rejected) or the hack of " +
        "reassigning an existing template's type via a code snippet after creation (tried for real, causes follow-on errors, not worth repeating). For a popup, `pageSettings` " +
        "covers width/close-button/background only — its trigger (on page load, on scroll, ...) has no ground truth yet and needs setting manually in the editor's Popup Settings " +
        '→ Triggers tab; always tell the user this after creating one, an untriggered popup never shows. On a multilingual site (Polylang/WPML), pass `lang` explicitly — templates ' +
        "otherwise land in whatever language Polylang defaults new elementor_library posts to (its own site default, not necessarily the language the template is actually for), " +
        "and there's no other way to set it at creation time.", {
        title: z.string().min(1),
