 ...
 * conversation's `site.select` would otherwise silently redirect
...
export function registerTools(server, deps = buildDefaultDependencies(process.env)) {
    ...
    server.tool('site.add', 'Register a new WordPress site for this tenant, storing its API credentials encrypted.', {
        url: z.string().url(),
        name: z.string().min(1),
    ...
    server.tool('site.list', 'List all WordPress sites registered for this tenant.', {}, async () => {
        try {
    ...
    server.tool('site.select', 'Set the active site for subsequent page/media/job tool calls.', { siteId: z.string().min(1) }, async (args) => {
        try {
    ...
    server.tool('site.update', 'Update a registered site\'s name, description, theme, or plugin list.', {
    ...
    server.tool('site.remove', 'Remove a registered site and its stored credentials.', { siteId: z.string().min(1) }, async (args) => {
        try {
    ...
    server.tool('site.healthCheck', 'Ping a registered site\'s /system endpoint and record whether it is reachable.', { siteId: z.string().min(1) }, async (args) => {
        try {
    ...
    const SITE_ID_PARAM = {
        siteId: z
            ...
            .min(1)
            .optional()
            .describe('Target site ID. Defaults to the site chosen via site.select. Pass explicitly when working across multiple sites so calls cannot land on the wrong one.'),
    };
    server.tool('page.create', 'Create a page on a site.', { title: z.string().min(1), content: z.string().optional(), status: z.string().optional(), ...SITE_ID_PARAM }, async (args) => {
        try {
    ...
    server.tool('page.get', 'Get a page from a site by ID.', { id: z.number().int().positive(), ...SITE_ID_PARAM }, async (args) => {
        try {
    ...
    server.tool('page.update', 'Update a page on a site.', {
    ...
    server.tool('page.delete', 'Delete a page on a site.', { id: z.number().int().positive(), ...SITE_ID_PARAM }, async (args) => {
        try {
            const client = await clients.forSite(tenantId, resolveSiteId(args.siteId, session));
    ...
    server.tool('page.list', 'List pages on a site.', { ...SITE_ID_PARAM }, async (args) => {
        try {
            const client = await clients.forSite(tenantId, resolveSiteId(args.siteId, session));
    ...
    server.tool('page.createFromHtml', 'Convert HTML into a native Elementor page (widgets, not an HTML blob) — this is what to use whenever there is a design/layout to build, not page.create. ' +
        'Async: returns { jobId, status: "queued", postId } immediately. Poll job.status(jobId) until its status is "completed" (or "failed"), then call ' +
        'page.update({ id: postId, status: "publish" }) — the converted page is always left as a draft, publishing is a separate step. Pass postId to convert into an existing page instead of creating a new one.', {
        html: z.string().min(1),
    ...
    server.tool('page.addElementorWidget', 'Append a specific Elementor widget (or a container with nested widgets) to an existing page — use this for widget types page.createFromHtml has no native mapping for ' +
        ...
        'scratch — use page.createFromHtml or page.create for that, then add specific widgets with this afterwards.', {
    ...
    server.tool('elementorTemplate.create', 'Create a conditional Elementor Theme Builder template (footer, header, 404, search-results, single-post, product, or popup) — for a request naming one of these, use this ' +
        "instead of page.create/page.addElementorWidget, which would only edit a single page's content, not a real site-wide/conditional template. Build `elements` the same way as " +
        'page.addElementorWidget\'s `element` (see ELEMENTOR_WIDGETS.md — for single-post/product templates, use context-only widgets like theme-post-content/woocommerce-*). ' +
        ...
        elements: z.array(elementorElementSchema).min(1).describe('Top-level Elementor elements — same shape as page.addElementorWidget\'s element.'),
        conditions: z
            .array(z.string())
            .optional()
    ...
    server.tool('elementorTemplate.get', 'Read back a Theme Builder template created via elementorTemplate.create (id, title, type, conditions, pageSettings, elements).', { id: z.number().int().positive(), ...SITE_ID_PARAM }, async (args) => {
        try {
            const client = await clients.forSite(tenantId, resolveSiteId(args.siteId, session));
    ...
    server.tool('elementorTemplate.updateConditions', 'Change which pages/context a Theme Builder template applies to, without touching its content.', {
        id: z.number().int().positive(),
    ...
    server.tool('elementorTemplate.updatePageSettings', 'Change a Theme Builder template\'s per-document settings (mainly popup width/close-button/background) without touching its content or conditions.', {
        id: z.number().int().positive(),
    ...
    server.tool('elementorKit.listColors', 'List the active Elementor Kit\'s Global Colors: the 4 system slots (primary/secondary/text/accent, always present) plus any custom colors. Speculative — see the tool\'s ' +
    ...
    server.tool('elementorKit.addColor', 'Add a custom Global Color to the active Elementor Kit (not one of the 4 fixed system slots — use elementorKit.updateColor for those). `color` must be a hex string ' +
    ...
    server.tool('elementorKit.updateColor', 'Update a Global Color by its _id (from elementorKit.listColors) — works for both system slots (primary/secondary/text/accent, color only, title is ignored) and custom colors.', { id: z.string().min(1), color: z.string().optional(), title: z.string().optional(), ...SITE_ID_PARAM }, async (args) => {
        try {
    ...
    server.tool('elementorKit.deleteColor', 'Delete a custom Global Color by its _id. Fails if the id belongs to a system slot (primary/secondary/text/accent) — those can only be updated, not deleted.', { id: z.string().min(1), ...SITE_ID_PARAM }, async (args) => {
        try {
            const client = await clients.forSite(tenantId, resolveSiteId(args.siteId, session));
    ...
    server.tool('elementorKit.listFonts', 'List the active Elementor Kit\'s Global Fonts: the 4 system slots (primary/secondary/text/accent) plus any custom fonts. Same speculative-shape caveat as ' +
        'elementorKit.listColors — verify in the Elementor editor.', { ...SITE_ID_PARAM }, async (args) => {
        try {
            const client = await clients.forSite(tenantId, resolveSiteId(args.siteId, session));
    ...
    server.tool('elementorKit.addFont', 'Add a custom Global Font to the active Elementor Kit. `typography` is a flat object of typography_*-prefixed settings (typography_typography, typography_font_family, ' +
    ...
    server.tool('elementorKit.updateFont', 'Update a Global Font by its _id (from elementorKit.listFonts) — works for both system slots (typography only, title is ignored) and custom fonts.', {
        id: z.string().min(1),
    ...
    server.tool('elementorKit.deleteFont', 'Delete a custom Global Font by its _id. Fails if the id belongs to a system slot (primary/secondary/text/accent) — those can only be updated, not deleted.', { id: z.string().min(1), ...SITE_ID_PARAM }, async (args) => {
        try {
            const client = await clients.forSite(tenantId, resolveSiteId(args.siteId, session));
    ...
    server.tool('form.createFluentForm', 'Build a brand-new Fluent Forms form (fields, submit button, settings, notifications) — for embedding a form that already exists instead, use the fluent-form-widget via ' +
        "page.addElementorWidget with { form_list: \"<id>\" }. Check this skill's FLUENT_FORMS.md for the field-tree shape, known element types, and how confident that reference " +
    ...
    server.tool('form.getFluentForm', 'Read back a Fluent Form created via form.createFluentForm (id, title, status, field tree).', { id: z.number().int().positive(), ...SITE_ID_PARAM }, async (args) => {
        try {
            const client = await clients.forSite(tenantId, resolveSiteId(args.siteId, session));
    ...
    server.tool('page.backup', 'Create a manual point-in-time snapshot of a page (content + Elementor data) on a site.', { id: z.number().int().positive(), reason: z.string().optional(), ...SITE_ID_PARAM }, async (args) => {
        try {
            const client = await clients.forSite(tenantId, resolveSiteId(args.siteId, session));
    ...
    server.tool('page.listBackups', 'List available backup snapshots for a page on a site, newest first.', { id: z.number().int().positive(), ...SITE_ID_PARAM }, async (args) => {
        try {
            const client = await clients.forSite(tenantId, resolveSiteId(args.siteId, session));
    ...
    server.tool('page.restore', 'Restore a page on a site from a previously created backup snapshot.', { backupId: z.string().min(1), ...SITE_ID_PARAM }, async (args) => {
        try {
            const client = await clients.forSite(tenantId, resolveSiteId(args.siteId, session));
    ...
    server.tool('audit.run', 'Run accessibility and SEO checks against a page on a site.', { id: z.number().int().positive(), ...SITE_ID_PARAM }, async (args) => {
        try {
            const client = await clients.forSite(tenantId, resolveSiteId(args.siteId, session));
    ...
    server.tool('media.upload', 'Upload a local file to a site\'s media library.', {
    ...
    server.tool('job.status', 'Check the status of an async job (e.g. HTML-to-Elementor conversion) on a site.', { jobId: z.string().min(1), ...SITE_ID_PARAM }, async (args) => {
        try {
            const client = await clients.forSite(tenantId, resolveSiteId(args.siteId, session));
