...
export function registerTools(server, deps = buildDefaultDependencies(process.env)) {
    ...
    server.tool('page_deleteElementorElement', "Remove an existing Elementor widget/container from a page entirely — the counterpart page_updateElementorWidget can't provide, since that one can only change an " +
        'existing element\'s settings, never remove it. Removes everything nested inside it too if it\'s a container with children. `elementId` is the element\'s own `id` ' +
        '(same one page_updateElementorWidget/page_addElementorWidget use — use page_getElementorData first if you need to find it, e.g. "remove the third text block on this ' +
        'page"). This is a real, immediate write, not a soft-delete/trash — call page_backup first if the user might want the removed content back, there is no undo for this ' +
        'call itself. Fails with a clear not-found error if no element with that id exists anywhere on the page — nothing is changed in that case.', {
        id: z.number().int().positive().describe('The page the element lives on.'),
        elementId: z.string().min(1).describe("The element's own id (not the page id)."),
        ...SITE_ID_PARAM,
    }, async (args) => {
        try {
            const client = await clients.forSite(tenantId, resolveSiteId(args.siteId, session));
            return jsonContent(await client.request('DELETE', `/wp-json/veilwright/v1/pages/${args.id}/elementor/elements/${args.elementId}`));
        }
        catch (error) {
            return errorContent(error);
        }
