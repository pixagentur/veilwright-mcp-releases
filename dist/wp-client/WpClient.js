...
export class WpApiError extends Error {
    ...
    /** Per-field validation errors from the plugin's envelope (`error.details`), e.g. `{ field, message }`. */
    details = []) {
        super(message);
        this.code = code;
        this.status = status;
...
export class WpClient {
    ...
    async parseEnvelope(response) {
        ...
        if (!envelope.success || envelope.error) {
            throw new WpApiError(envelope.error?.code ?? 'VW_UNKNOWN', envelope.error?.message ?? 'Unknown error', response.status, Array.isArray(envelope.error?.details) ? envelope.error.details : []);
        }
