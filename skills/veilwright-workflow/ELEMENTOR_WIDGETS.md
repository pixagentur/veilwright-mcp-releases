...
## Migrating from another page builder (WPBakery → Elementor) — real conversion, confirmed live (michael-fenster.de, 2026-08-21)

For "rebuild this WPBakery/other-builder page as a native Elementor page" — see `SKILL.md` section 3c for the full workflow checklist (read an existing template first, read the source page's *raw* stored shortcodes, build atomically, verify against the original afterward). This section is the shape mapping itself, confirmed against a real WPBakery → Elementor conversion:

```
vc_row                  → container (flex_direction: "column", content_width: "boxed")
vc_row + N× vc_column   → container (flex_direction: "row")
                            └── container (width: "<percent>%")   ← one per WPBakery "column"
                            └── container (width: "<percent>%")
vc_column_text          → text-editor (+ a separate heading widget for any H1–H6 inside it)
vc_separator            → divider (horizontal)
vc_separator (vertical) → divider (vertical — see the Divider entry above for the geometry)
vc_single_image / <img> → image (external URL: {"url": "...", "id": ""}, no attachment needed)
<a class="btn...">      → button (not a raw <a> left inside a text-editor — see SKILL.md's text-editor rule)
[wpseo_breadcrumb]      → breadcrumbs (Elementor Pro's own widget, see the Breadcrumbs entry above)
[rank_math_breadcrumb]  → breadcrumbs (same widget, RankMath is just a different SEO plugin's shortcode for the same idea)
[contact-form-7] etc.   → the matching dedicated form widget/tool if the site has one (see section 5f of SKILL.md), otherwise a shortcode widget as a last resort
```

**Column width lives on the child container as `width` (a percentage), not `_column_size`** (WPBakery's own attribute name — don't carry it over, it means nothing to Elementor). Minimal two-column example (2/3 + 1/3 split, matching a real `vc_row` with two `vc_column`s):
```json
{ "elType": "container", "settings": {
  "content_width": "boxed",
  "flex_direction": "row",
  "flex_gap": { "unit": "px", "size": 35, "column": "35", "row": "35", "isLinked": true },
  "flex_wrap": "wrap"
}, "elements": [
  { "elType": "container", "settings": { "content_width": "full", "width": { "unit": "%", "size": 66 } }, "elements": [ /* ... */ ] },
  { "elType": "container", "settings": { "content_width": "full", "width": { "unit": "%", "size": 33 } }, "elements": [ /* ... */ ] }
]}
```
Responsive column widths use the standard per-breakpoint suffix on the same child container (`width_mobile_extra`, `width_mobile`, ...) — read an existing responsive container on the target site first (a footer template is a reliable real example) rather than guessing values. Row spacing is `flex_gap` on the parent, not WPBakery's `gap: "extended"`-style setting.
...
### Icon List — ✅ VERIFIED (real reference section, hand-built in the Elementor editor, 2026-07-15)
`page_createFromHtml` maps a plain `<ul>`/`<nav>` to this widget automatically — fine for an unstyled list. **But when the list is part of the actual design** (a pricing card's feature checklist, anything with a deliberately-sized/colored/spaced icon) **build it directly with the widget's own settings, don't rely on the HTML conversion** — a converted `<ul>` won't carry icon size/spacing/color/typography the way the widget's real settings do, and (per the general `page_createFromHtml` CSS-fidelity caveat above) styling on converted content is exactly the kind of thing that reliably gets lost. Real verified shape:
```json
{ "elType": "widget", "widgetType": "icon-list", "settings": {
  "icon_list": [
    { "text": "Free shipping", "selected_icon": { "value": "fas fa-check-circle", "library": "fa-solid" }, "_id": "abc1234" },
    { "text": "30-day returns", "selected_icon": { "value": "fas fa-check-circle", "library": "fa-solid" }, "_id": "def5678" }
  ],
  "space_between": { "unit": "px", "size": 6, "sizes": [] },
  "icon_align": "start",
  "icon_size": { "unit": "px", "size": 18, "sizes": [] },
  "text_indent": { "unit": "px", "size": 6, "sizes": [] },
  "icon_self_align": "left",
  "icon_self_vertical_align": "flex-start",
  "icon_vertical_offset": { "unit": "px", "size": 5, "sizes": [] },
  "icon_typography_typography": "custom",
  "icon_typography_font_family": "Noto Sans",
  "icon_typography_font_size": { "unit": "px", "size": 16, "sizes": [] },
  "icon_typography_font_weight": "400",
  "icon_typography_line_height": { "unit": "px", "size": 24, "sizes": [] },
  "__globals__": { "icon_color": "globals/colors?id=accent" }
}}
```
Each list item's own `_id` is Elementor's internal repeater-row id — safe to invent a short hex-ish string per item, doesn't need to match anything (confirmed: the real export reused the same three `_id`s verbatim across three different pricing cards, meaning they don't need to be globally unique either). `icon_vertical_offset` nudges the icon down slightly to visually align with the first line of multi-line text — useful when `icon_self_vertical_align: "flex-start"` on a wrapped long list item.

### Divider — ✅ VERIFIED (real export, 2026-07-03; vertical-divider geometry confirmed live 2026-08-21)
```json
...
**This is also the right widget for a vertical divider** (a short thick rule used decoratively between two inline elements, e.g. in a header top bar) — not `html`, a Spacer widget, a container border, or an image. The two size controls map to the *opposite* axis from what their names suggest at first glance:
- `width` (Content tab → "Width" in the Elementor UI) = the divider's **length** — small for a vertical rule, e.g. `3px`.
- `weight` (Style tab → "Weight") = the divider's **thickness** — large for a vertical rule, e.g. `60px`.
```json
{ "elType": "widget", "widgetType": "divider", "settings": {
  "style": "solid",
  "width": { "unit": "px", "size": 3 },
  "weight": { "unit": "px", "size": 60 },
  "align": "center",
  "gap": { "unit": "px", "size": 2 },
  "__globals__": { "color": "globals/colors?id=primary" }
}}
```
A **horizontal** divider uses the same widget with the ratio inverted — `width` large (typically a `%` value spanning the available space) and `weight` small (1–3px). `align` (`left`/`center`/`right`) and `gap` (spacing to whatever sits next to it) apply either way.
...
**Responsive**: don't assume the *number* of columns needs to change per breakpoint — in the reference, the row stays 3-across down to tablet width (768px) purely because 3× a 30%-wide card plus gaps still fits; it only collapses to 1-per-row once `flex_wrap` actually has to wrap. The only breakpoint-specific setting used was `flex_gap_mobile` (a smaller, fixed 20px gap once stacked) — no `width_mobile`/`width_tablet` override was needed for this 3-card case. For a row with more columns that needs an earlier breakpoint change, add `width_tablet`/`width_mobile` overrides on the child the same way (Elementor's per-breakpoint `_<breakpoint>` suffix convention — `width_widescreen`/`width_laptop`/`width_tablet_extra`/`width_tablet`/`width_mobile_extra`/`width_mobile`). A second real reference (a 3-tier pricing row) additionally used `flex_direction_mobile_extra: "column"` on the parent and `width_mobile_extra: {unit:"%", size:100}` on each card, to force a clean full-width single column below a specific breakpoint rather than relying on wrap alone — an alternative worth reaching for when wrap-based stacking doesn't collapse cleanly enough on its own.

**A real, confirmed Elementor bug when a `width_tablet` (or other mid-range breakpoint) override is added: the base `width` can silently win back over it at exactly that breakpoint, due to CSS source order, not anything wrong in the settings themselves.** Found live (Metall-Service-Dreher Impressum, 2026-08-17): a card row with `width: 30%` + `width_tablet: 48%` visually still showed 3 narrow columns at a real 768px viewport instead of 2 wider ones. Read the actual generated CSS (`wp-content/uploads/elementor/css/post-<id>.css`) rather than guessing — it contained, **in this order**: `@media(max-width:1024px) and (min-width:768px){ ...--width:48%; }` (the tablet override) followed *later in the file* by `@media(min-width:768px){ ...--width:30%; }` (the base value, which Elementor expresses as "anything above the mobile breakpoint" rather than emitting unconditionally). Both media queries match at a real tablet viewport; same specificity; **later-in-source wins** — so the base rule silently overrides the tablet-specific one it was supposed to be overridden by. This isn't a caching issue (confirmed by reading the live CSS file directly, matching the exact edit) and isn't specific to a custom breakpoint value (`viewport_tablet: 1024` here is Elementor's own stock default). **Mitigation, unverified but logical given the mechanism:** also set an explicit value at the next breakpoint up from the one being overridden (e.g. `width_laptop` matching the base value) rather than leaving a bare `width` to serve as an open-ended "everything above mobile" fallback — this should make Elementor emit a properly bounded range for the base value too instead of the broad `min-width` query that outranks the in-between override. Tell the user about this rather than silently "fixing" their live site's data without confirming — it's their content, and the mitigation hasn't been round-trip verified yet.

**None of this is a fixed template to reproduce verbatim on every site.** It's the verified *mechanism* (percentage width + `content_width:"full"` + flex-wrap) for "several same-size boxes side by side" — exact widths, gaps, colors, breakpoints, and column counts should always come from whatever design/HTML/instructions the user actually gave for that page, not be copied wholesale from this reference. Read the real design first, then reach for this mechanism to implement it.

**Featured/highlighted card variant** (e.g. a pricing row's recommended tier) — verified from the same reference: no special widget, just distinguish the one card's own container settings from its siblings — a thicker, brand-colored border (`border_width: 2px` + a brand hex/global color, vs. `1px` + a neutral grey on the other cards) is what the reference used, though a permanently-visible box-shadow (see above) is an equally valid alternative. Pair with `flex_justify_content: "space-between"` on **every** card's own `flex_direction:"column"` layout (not just the featured one) so each card's CTA button lands at the same vertical position across the row even when the amount of text above it differs — the button gets pushed to the bottom of the available height instead of sitting directly under a shorter/longer text block.

**Overlapping badge/ribbon (e.g. "BELIEBT"/"Popular")** — verified from the same reference: a plain `heading` widget (`header_size: "div"` — used purely as a styled label, not a real heading level) placed as the *first* child inside the featured card's container, using Elementor's Advanced-tab **Position** controls to lift it out of normal flow and overlap the card's own top border:
```json
{ "elType": "widget", "widgetType": "heading", "settings": {
  "title": "Beliebt",
  "header_size": "div",
  ...
  "_padding": { "unit": "px", "top": "5", "right": "12", "bottom": "5", "left": "12", "isLinked": false },
  "_position": "absolute",
  "_offset_orientation_h": "end",
  "_offset_x_end": { "unit": "px", "size": 30, "sizes": [] },
  "_offset_y": { "unit": "px", "size": -15, "sizes": [] },
  "_background_background": "classic",
  "_border_radius": { "unit": "px", "top": "20", "right": "20", "bottom": "20", "left": "20", "isLinked": true },
  "__globals__": { "title_color": "globals/colors?id=accent", "_background_color": "globals/colors?id=accent" }
}}
```
`_position: "absolute"` plus `_offset_x_end`/`_offset_y` (or `_offset_x_start`, depending on `_offset_orientation_h`/`_offset_orientation_v`) is the general Elementor mechanism for pulling any widget out of normal flow and pinning it relative to its parent container — a small negative `_offset_y` is what makes it visually poke above the card's edge rather than sit flush inside it. Same underscore-prefixed Advanced-tab convention as `_element_id`/`_attributes`/`custom_css` documented earlier in this file.

**Container labeling (`_title`)** — set a short, meaningful `_title` on every container you build (e.g. `"Preise"`, `"Spalten"`, `"Single Site"` rather than leaving it unset/default) — purely cosmetic (shows in the Elementor editor's Structure/Navigator panel, no effect on rendering), but it's what makes a page buildable further by hand afterward without having to click into every nameless container to find the right one. Confirmed as the site owner's own convention across multiple hand-built reference sections.
...
### Loop Grid / Portfolio
Both reference a separately-saved Elementor "Loop Item" template by ID (`template_id` in settings) that has to already exist. **Fixed 2026-09-11: `elementorTemplate_create({ type: "loop-item" })` builds this** — no longer editor-only, see `content-type-sections` (resource) for the full sequence. Without one, the widget renders empty even with correct settings. Real exports (Elementor Pro, WooCommerce product listings) confirm the shape once a template exists:
```json
...
```
Also references a pre-existing Loop Item template (`template_id`) to render results — build one first via `elementorTemplate_create({ type: "loop-item" })`, same as Loop Grid/Portfolio.

### Table of Contents (Elementor Pro) — ✅ VERIFIED twice, real fuller shape confirmed 2026-08-17
```json
{ "elType": "widget", "widgetType": "table-of-contents", "settings": {
  ...
  "no_headings_message": "Es wurden keine Überschriften auf dieser Seite gefunden.",
  "word_wrap": "ellipsis",
  "collapse_subitems": "yes",
  "border_width": { "unit": "px", "size": 1, "sizes": [] },
  "border_radius": { "unit": "px", "size": 0, "sizes": [] },
  "header_separator_width": { "unit": "px", "size": 1, "sizes": [] },
  "padding": { "unit": "px", "size": 15, "sizes": [] },
  "heading_gap": { "unit": "px", "size": 5, "sizes": [] },
  "list_typography_typography": "custom",
  "list_typography_font_family": "Open Sans",
  "list_typography_font_size": { "unit": "px", "size": 16, "sizes": [] },
  "list_typography_font_weight": "400",
  "list_typography_line_height": { "unit": "em", "size": 1.65, "sizes": [] },
  "list_indent": { "unit": "px", "size": 15, "sizes": [] },
  "item_text_hover_transition_duration": { "unit": "ms", "size": 300, "sizes": [] },
  "item_text_underline_active": "yes",
  "__globals__": {
    "border_color": "globals/colors?id=...",
    "loader_color": "globals/colors?id=...",
    "header_text_color": "globals/colors?id=...",
    "toggle_button_color": "globals/colors?id=...",
    "item_text_color_normal": "globals/colors?id=...",
    "marker_color": "globals/colors?id=..."
  }
}}
```
Auto-generates its list from the page's actual `<h1>`–`<h6>` headings at render time — no fixed content to configure. `headings_by_tags` restricts which levels it picks up (`["h2", "h3"]` is the common case: skip the page's own `h1` and any decorative `div`-rendered eyebrow headings). Every color setting here is a real Kit-color-global candidate (`__globals__`), same as any other widget — style it to match the section it sits in rather than leaving Elementor's own defaults. First confirmed (thinner shape) on a page that also used `menu-anchor` widgets for jump targets; second confirmation (this fuller shape, real production use) on a Datenschutzerklärung page, placed directly in the hero next to the `h1`/intro text rather than below the fold — see `PRIVACY_POLICY.md` §5.2 for the full placement pattern (two-column hero, `collapse_subitems` for a long section list, real heading-hierarchy semantics preserved even when the headings themselves are styled small/quiet).
...
**Still not creatable**: Cookie Consent, Preferences Banner. Same limitation as Off-Canvas. (Loop Item and Archive were in this list too — **fixed 2026-09-11**, both are real `elementorTemplate_create` types now, see `content-type-sections`.)
