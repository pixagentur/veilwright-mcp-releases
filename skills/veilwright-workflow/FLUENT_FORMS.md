# Fluent Forms reference for `form.createFluentForm`

Use this when a request needs a **brand-new** Fluent Form built (a contact form, a request form, ...) — as opposed to embedding a form that already exists on the site, which is the `fluent-form-widget` Elementor widget (`{ "form_list": "<id>" }`, see `ELEMENTOR_WIDGETS.md`). Call `form.createFluentForm({ title, fields, ..., siteId })`.
