...
1. **Optional but recommended first:** if the design defines a brand color palette/typography used across multiple pages, set them up once via `elementorKit_addColor`/`elementorKit_addFont` (and `elementorKit_listColors`/`elementorKit_listFonts` to check what's already there) — this makes every subsequent page/template built with `page_createFromHtml`/`elementorTemplate_create` able to reference the same global tokens instead of hardcoding colors per page.
...
4. **Navigation menu:** there is currently **no MCP tool for this** — the plugin's REST API has menu endpoints (`/menus`, `/menus/{id}/items`) but no `menu_*` MCP tool wraps them yet. Say so explicitly rather than trying to fake a menu via a page or Theme Builder header — tell the user the menu still needs to be built manually in WP-admin → Appearance → Menus, and flag it as a known gap if they want it added.

Report each piece's outcome separately (page IDs, footer/header template IDs) rather than one blanket "site built" summary — that's what makes a partial failure (e.g. the footer template silently not saving correctly) visible instead of hidden inside an overall success claim.
