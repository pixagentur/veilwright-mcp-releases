...
### 5.1 Bekannte Werkzeug-Einschränkung — betrifft genau diesen Anwendungsfall

Eine Datenschutzerklärung ist fast immer eine **Ersetzung** einer bestehenden Seite, kein Neuanlegen. Das kann heute keines der Page-Tools sauber:

| Problem | Auswirkung |
|---|---|
| `page_addElementorWidget` hängt ausschließlich an | Die alte Fassung bleibt stehen, die Seite enthält die Erklärung doppelt |
| `page_updateElementorWidget` kann Kindelemente nicht ändern | Ein bestehender Block lässt sich nicht durch einen neuen ersetzen |
| `page_update` schreibt nur `post_content` | Auf Elementor-Daten kein Zugriff |
| kein Element-Delete-Tool | Aufräumen nur von Hand im Editor |
| `page_listBackups` liefert oft eine leere Liste, obwohl `page_backup` eine `backupId` zurückgibt | Das Backup ist nicht zuverlässig auffindbar — vor einer Ersetzung trotzdem `page_backup` aufrufen und die `backupId` dem Nutzer explizit mitteilen, statt sich auf `page_listBackups` zu verlassen |

**Interims-Workaround, der real funktioniert:** den alten Inhalts-Container über `page_updateElementorWidget` auf **allen** Breakpoints ausblenden (`hide_desktop`, `hide_laptop`, `hide_tablet_extra`, `hide_tablet`, `hide_mobile_extra`, `hide_mobile` — siehe die reale Mechanik in `SKILL.md`'s Header/Footer-Abschnitten) und den Elementtitel (`_title`) auf etwas wie „ALT — zum Löschen markiert, ersetzt am {Datum}" setzen, dann den neuen Inhalt daneben/danach mit `page_addElementorWidget` aufbauen. Die sichtbare Dopplung ist damit für Besucher sofort weg.

**Aber:** ausgeblendete Inhalte werden weiterhin ausgeliefert und stehen per `display:none` weiterhin im DOM — für Screenreader und Suchmaschinen also weiterhin vorhanden. Das ist eine Notlösung für die sichtbare Dopplung, kein Ersatz für echtes Löschen. **Dem Nutzer explizit sagen, dass der alte Block nur versteckt, nicht entfernt ist**, und dass ein Aufräumen im Elementor-Editor von Hand nötig bleibt, bis es ein Element-Delete-Tool gibt.
...
2. `page_replaceElementorData(siteId, id, elements)` — ersetzt den Elementor-Baum vollständig, löst §5.1 in einem Aufruf statt über den Ausblend-Workaround.
3. `page_deleteElementorElement(siteId, id, elementId)` — entfernt ein Element samt Kindern.
