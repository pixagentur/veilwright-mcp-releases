...
### 5.3 Kurze Fakten-Blöcke als responsives Card-Grid, lange Abschnitte volle Breite — real verifiziert (Metall-Service-Dreher, Impressum, 2026-08-17)

**Gilt allgemein für jede rechtliche Content-Seite mit gemischt kurzen und langen Abschnitten (Impressum, Datenschutzerklärung, AGB, ...) — hier am Impressum gefunden, nicht Datenschutz-spezifisch.** Der Seitenbetreiber hat sein Impressum nach demselben Muster wie zuvor die Datenschutzerklärung überarbeitet (§5.2), diesmal zusätzlich mit einer echten Layout-Struktur statt reiner Typografie-Zurückhaltung:

1. **Kurze Fakten-Blöcke** (Anschrift, Kontakt, Vertreten durch, Registereintrag, USt-ID, Verantwortlich — je nur eine Überschrift + 2-4 Zeilen Text) wurden in ein **responsives Card-Grid** gruppiert: jeder Block ein eigener Container mit `width: 30%` (Desktop, 3 Karten pro Zeile), `width_tablet: 48%` (2 pro Zeile — siehe aber die echte Cascade-Falle dazu in `ELEMENTOR_WIDGETS.md`s Multi-column-row-Abschnitt), volle Breite auf Mobile (leerer `width_mobile`-Wert + `flex_wrap` lässt es natürlich stapeln). Gleicher Mechanismus wie das dort dokumentierte Multi-column-row-Pattern (`flex_direction:"row"` + `flex_wrap:"wrap"` auf dem Elternelement, `width:{unit:"%"}` + `content_width:"full"` auf jedem Kind), hier auf reale Fakten-Blöcke statt Feature-Karten angewendet — bestätigt, dass das Pattern nicht auf Marketing-Inhalte beschränkt ist.
2. **Lange Abschnitte** (Haftung für Inhalte, Haftung für Links, Urheberrecht, Datenschutz-Verweis, Design-Credit) blieben **volle Breite, ein Container pro Abschnitt**, kein Card-Grid — lange Fließtext-Absätze in eine schmale Spalte zu zwingen hätte die Lesbarkeit verschlechtert, nicht verbessert. **Regel: kurze Fakten gruppieren, lange Prosa nicht.** Nicht pauschal "alles ins Grid" oder "alles volle Breite" — die Entscheidung hängt vom tatsächlichen Inhalt jedes einzelnen Abschnitts ab.
3. Gleicher zurückhaltender Überschriften-Stil wie in §5.2 (custom Typografie, kein Kit-Global, `header_size` unverändert bei `h2`) wurde unverändert wiederverwendet — bestätigt, dass das der jetzt etablierte Stil für diese Seitenart ist, nicht ein Einmal-Experiment.
4. **Echte Falle beim Kopieren von Containern:** beim Duplizieren des Karten-Containers für die sechs Fakten-Blöcke blieb `_title` (das Navigator-Panel-Label aus §5.2, Punkt 2) bei den meisten Kopien auf dem Wert der ersten Kopie stehen ("Haftung Links" — ein Label, das ursprünglich zu einem ganz anderen Abschnitt gehörte) statt für jede Kopie umbenannt zu werden. Der eigentliche Seiteninhalt/die Anzeige ist davon nicht betroffen (rein kosmetisch fürs Editor-Navigator-Panel), aber der Zweck des `_title`-Labels (lange Seiten im Editor navigierbar machen) geht dadurch verloren. **Beim Duplizieren eines Containers für ein sich wiederholendes Layout: `_title` bei jeder Kopie einzeln setzen, nicht vom Original übernehmen lassen.**
5. Kein Inhaltsverzeichnis-Widget diesmal (anders als bei der Datenschutzerklärung) — sinnvoll: ein Impressum ist überwiegend kurze Fakten, die das Card-Grid selbst schon scannbar macht; ein Inhaltsverzeichnis lohnt sich eher bei überwiegend langer Fließtext-Seite wie der Datenschutzerklärung. Keine feste Regel, sondern situativ je nach Seitenanteil kurz/lang zu entscheiden.

**Auf Tablet-Breite (768px) real geprüft, nicht nur Desktop/Mobile:** siehe der echte Elementor-CSS-Cascade-Bug in `ELEMENTOR_WIDGETS.md`s Multi-column-row-Abschnitt (Punkt 1 oben) — dort per generierter CSS-Datei bestätigt, nicht nur vermutet, und dem Nutzer gemeldet statt eigenmächtig an dessen Live-Daten korrigiert.
